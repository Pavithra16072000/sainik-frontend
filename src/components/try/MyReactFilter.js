...
const MyReactFilter = () => {
    ...
    const [allSNo,setAllSNo] = useState([]);
    const [allSNo2,setAllSNo2] = useState([]);

    const [filteredSNo,setFilteredSNo] = useState(allSNo);
    const [filteredSNo2,setFilteredSNo2] = useState(allSNo2);
    ...
    const getSNo = async () => {
      ...
      // const query =  `where:{` {Attribute} `:`{Name}`}`
        const response = await axiosJWT.get('http://localhost:5000/reactfilter'//,{
      ...
      setAllSNo(response.data);
      setFilteredSNo(response.data);
      // setm(response.data);

    }
    const getSNo2 = async () => {
        const response = await axiosJWT.get('http://localhost:5000/reactfilter2'//,{

      );
      setAllSNo2(response.data);
      setFilteredSNo2(response.data);
    ...
    let result = [];
    console.log(value);
    result = allSNo.filter((data) => {
    return data.Service_Name.search(value) != -1;
    });
    // setAllSNo(result);

    setFilteredSNo(result);
    //setAllSNo(filteredSNo);

setm(filteredSNo)
    }
    const handleCorps = (event) =>{
      let value = event.target.value;
    let result = [];
    console.log(value);
    result = allSNo.filter((data) => {
    return data.Corps_Name.search(value) != -1;
    });
    setFilteredSNo(result);
   setAllSNo(filteredSNo);
  setm(filteredSNo)
    ...
    const handleRecordOffice = (event) =>{
      let value = event.target.value;
    let result = [];
    console.log(value);
    result = allSNo.filter((data) => {
    return data.Record_Office_Name.search(value) != -1;
    });
    setFilteredSNo(result);
  setAllSNo(filteredSNo);
  setm(filteredSNo)
    ...
    console.log(value);
    result = allSNo.filter((data) => {
    return data.Group_Name.search(value) != -1;
    });
    setFilteredSNo(result);
  setAllSNo(filteredSNo);
  setm(filteredSNo)
    ...
    <div class="card example-2 scrollbar-ripe-malinka">
      ...
      <h1>{Sum}</h1>
          <tr>
              <th>Sl.No</th>
              <th>Service_No </th>
              <th>Service_Name<input type="text" onChange={(event) =>handleSName(event)} />
</th>
              <th>Corps_Name<input type="text" onChange={(event) =>handleCorps(event)} />
</th>
              <th>Record_Office_Name<input type="text" onChange={(event) =>handleRecordOffice(event)} /></th>
              <th>Group_Name<input type="text" onChange={(event) =>handleGroup(event)} /></th>
              <th>Trade_Name</th>
              <th>Rank_Name</th>
              <th>Rank_Category</th>
              <th>Name</th>
