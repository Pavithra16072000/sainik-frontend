...
const EditDep= async (e) => {
            e.preventDefault();

    try{
         localStorage.setItem('A_Dep_Name',ADep_Name);
        navigate('/Form8');
    }catch (error) {
            if (error.response) {
...
//form2
                ...
                Disability_Percentage: localStorage.getItem('Disability_Pension'),
...
*/}
    ...
    <tbody>
        {dep.map((user, index) => (
           <tr key={user.id}>
                ...
                <td>
                <button onMouseEner={() =>setADep_Name(user.Dep_Name)} onClick={EditDep} >EDIT</button></td>
                  <td>
                <button onClick={Form7Delete} > DELETE </button></td>
            </tr>
   ))}
 ...
 {dep.map((user, index) => (
          ...
          <div className="card-header color"key={user.id}>
                  <h3 className='color'> <strong> {user.Relation} </strong></h3>
           </div>
            <div class="card-body row">

           <span class="card-text col-sm-3"value={Dep_Name} onChange={(e) =>setDep_Name(user.Dep_Name)}>{user.Dep_Name}</span>
<span className="card-title col-sm-4">
           <table className="table  is-fullwidth">
             ...
             <button className="btn my-2 my-sm-0 col-sm-1 view-align" type="submit" onMouseEnter={() =>setADep_Name(user.Dep_Name)}>View </button>
           </div>
           </div>
           </form>
